    ...
    context 'when resource not found' do
      before do
        allow(connection.raw_connection)
          .to receive(:get)
          .with(path, nil)
          .and_raise(Faraday::ResourceNotFound, 'foo')
      end
    ...
    context 'when connection failed' do
      before do
        allow(connection.raw_connection)
          .to receive(:get)
          .with(path, nil)
          .and_raise(Faraday::ConnectionFailed, 'foo')
      end
    ...
    context 'when client error' do
      before do
        allow(connection.raw_connection)
          .to receive(:get)
          .with(path, nil)
          .and_raise(Faraday::ClientError.new('foo', status: 500))
      end
    ...
    context 'when unprocessable entity' do
      before do
        allow(connection.raw_connection)
          .to receive(:post)
          .with(params)
          .and_raise(Faraday::ClientError.new('foo', status: 422))
      end
